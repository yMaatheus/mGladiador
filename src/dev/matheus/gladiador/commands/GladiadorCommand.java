...
public class GladiadorCommand extends Command {
	...
	public boolean execute(CommandSender sender, String arg, String[] args) {
			...
			if (args.length == 1) {
				if ((args[0].equalsIgnoreCase("sair"))) {
					...
					for (int a = 0; a < m.size(); a++) {
						...
						sender.sendMessage(" §f" + position + "§7. §b" + clan + " §7- §f" + b.getWins() +" vit§rias");
					}
					sender.sendMessage("");
					...
					if (sender.hasPermission("mgladiador.admin")) {
						...
						if (gladiadorManager.getGladiador().getStatus() != statusType.PVP) {
							sender.sendMessage("§cSo é possivel forçar o inicio do deathmatch durante o status de PvP.");
							return false;
						}
					...
					if (sender.hasPermission("mgladiador.admin")) {
						if (sender instanceof Player) {
							...
							if (find) {
								...
								sender.sendMessage("§77Localiza§§o §f" + name.toLowerCase() + " §7definida com sucesso!");
							}
						}
						return true;
					}
		...
		if (sender instanceof Player) {
			...
			if (gladiador.getParticipantes().containsKey(player)) {
				sender.sendMessage("§cVoc§ j§ est§ no evento gladiador.");
				return true;
			}
	...
	private void sendHelpCommands(CommandSender sender, String arg) {
		...
		if (sender.hasPermission("mgladiador.admin")) {
			...
			sender.sendMessage(cmd+" top set (tag) (quantidade) §f- §7Defina a quantidade de vit§rias de um clan.");
			sender.sendMessage(cmd+" set (spawn | saida | deathmacth) §f- §7Salve as localiza§oes.");
			sender.sendMessage(cmd+" info §f- §7Obtenha informa§§es do evento gladiador.");
			sender.sendMessage("");
		}
	}
	...
	private void sendInfoGladiador(CommandSender sender) {
		...
		if ((s == statusType.CHAMANDO) || (s == statusType.FECHADO)) {
			sender.sendMessage("§7Tempo§8: §fAinda ira comecar a contagem!");
		} else if ((s == statusType.DEATHMATCH_PVPOFF) || (s == statusType.DEATHMATCH_PVPON)) {
			sender.sendMessage("§7Tempo§8: §fDeathmatch ja encontra-se em andamento.");
		} else {
